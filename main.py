...
def init_db():
    drop_db('all')
    users = get_users_from_server(BASE_URL, HEADER_dict)
    add_users(users)
    add_groups(get_groups_from_server(BASE_URL, HEADER_dict))
    add_user_in_groups(users)
    orgs = get_orgs_from_server(BASE_URL_ORG, HEADER_dict)
    add_orgs(orgs)
    global users_from_db, groups_from_db
...
def add_and_set_users():
    if not is_org:
        user_list, treedata = get_user_list_with_org(selected_group['organization_id'])
        tree2.update(treedata)
        users_for_group = get_users_for_group_from_db_with_org(selected_group)
        user_for_group_ids = []
        for user_for_group in users_for_group:
            user_for_group_ids.append(user_for_group['id'])
        all_user_ids = []
        for user_from_all in user_list:
...
def get_users_for_group_from_db_with_org(group):
    ...
    users_for_tree = list()
    for user in users:
        user_for_tree = {'id': user[0],
                          'login': user[1],
                          'name': user[2],
                          'is_blocked': user[3]}
        users_for_tree.append(user_for_tree)
    con.close()
    return users_for_tree
...
def get_user_list_with_org(org_id):
    user_treedata = sg.TreeData()
    us_list = []
    if users_from_db:
        for us_from_db in users_from_db:
            if org_id == us_from_db['organization_id']:
                us_list.append([us_from_db['id'], us_from_db['login'], us_from_db['name']])
                if us_from_db['is_blocked']:
                    us_list[-1].append(u'\u0020\u0020\u0020' + SYMBOL_X_SMALL)
                else:
                    us_list[-1].append('')
                user_treedata.insert('', us_from_db['id'], '',
                                      values=[us_from_db['login'],
                                              us_from_db['name'],
                                              (u'\u0020\u0020\u0020' + SYMBOL_X_SMALL) if us_from_db[
                                                  'is_blocked'] else ''],
                                      # icon=check[0]
                                      )
    return us_list, user_treedata
...
def make_main_window(ip):
    ...
    # user_list = list()
    treedata_empty = sg.TreeData()
    user_list_treedata, user_list_treedata_2 = get_user_list_treedata_full()
    group_list_treedata, group_list_treedata_2 = get_group_list_treedata_full()
    ...
    tab2_layout = [
        [sg.Frame('Группы',
                  [
                      ...
                      [sg.Tree(group_list_treedata,
                               headings=['id', 'Имя', 'Описание', 'Э', 'Блок'],
                               visible_column_map=[False, False, True, False, False],
                               col0_width=15,
                               col0_heading="Имя",
                               # col_widths=[0, 0, 10, 4, 4, 4, 4],
                               num_rows=10,
         ...
         sg.Frame('Пользователи', [[sg.Column([[
             ...
             [
                 sg.Tree(
                         # data=user_list_treedata_2,
                         data=treedata_empty,
                         headings=['Логин', 'Имя', 'Блок'],
                         col0_width=5,
                         col_widths=[15, 20, 5],
                         num_rows=10,
                         key='-TREE2-',
...
def make_organizations():
    # org = get_orgs_from_server(BASE_URL_ORG, HEADER_dict)
    # orgs = get_orgs_from_db()
    orgs_list = get_orgs_from_db_list()
    layout = [
        [
...
def update_all():
    ...
    window['-TREE2-'].update(treedata_empty)
...
def set_buttons_disabled(param=True):
    ...
    window['-СollapseExpandUser-'].update(disabled=param)
    window['-СollapseExpandGroup-'].update(disabled=param)
    ...
    window['-groups-tree-'].update(visible=not param)
...
def set_disabled_group_modify():
    window_modify_group['GroupModifyName'].update(disabled=True)
    window_modify_group['GroupModifyDesc'].update(disabled=True)
    window_modify_group['GroupEditOrg'].update(disabled=True)
    window_modify_group['GroupModifyEmergency'].update(disabled=True)
        ...
        if ev_login == "OK button":
            ...
            if validate('login'):
                if ip == '127.0.0.1':  # TODO
                    ...
                    # print(f'DEF1A = {DEF1A}')
                    try:
                        path_pwd = "/home/" + USERNAME + "/Omega/.admPWD_b"
                        with open(path_pwd, mode='r') as pwd_hash_file:
                ...
                while True:
                    ...
                    while True:
                        ...
                        if event == '-groups-tree-':
                            if values['-groups-tree-']:
                                is_org = check_org(values['-groups-tree-'][0])
                                if filter_status:
                                    # noinspection PyUnboundLocalVariable
                                    selected_group = filtered_users_list_of_dict[values['-users-'][0]]
                                else:
                                    if not is_org:
                                        selected_group_id = str(values['-groups-tree-'][0]).partition('.')[0]
                                        selected_group = get_group_by_id_from_db(selected_group_id)
                                    else:
                                ...
                                if not is_org:
                                    ...
                                    window['-checkAllUsers-'].update(False, disabled=False)
                                else:
                                    window['-DelGroup-'].update(disabled=True)
                                    window['-BlockGroup-'].update(image_data=ICON_BLOCK_BASE_64_BLUE, disabled=True)
                                    window['-BlockGroup-'].TooltipObject.text = 'Заблокировать'
                                    window['-checkAllUsers-'].update(False, disabled=True)
                        ...
                        if event == '-СollapseExpandGroup-':
                            group_list_treedata = get_group_list_treedata()
                            if window['-СollapseExpandGroup-'].metadata == 'collapse':
                                window['-СollapseExpandGroup-'].update(image_data=ICON_EXPAND_32_BASE_64_BLUE)
                                for key in group_list_treedata.tree_dict:
                                    window['-groups-tree-'].Widget.item(key_to_id(key, window['-groups-tree-']), open=False)
                                window['-СollapseExpandGroup-'].metadata = 'expand'
                                window['-СollapseExpandGroup-'].TooltipObject.text = 'Развернуть'
                            else:
                                window['-СollapseExpandGroup-'].update(
                                    image_data=ICON_COLLAPSE_32_BASE_64_BLUE)
                                for key in group_list_treedata.tree_dict:
                                    window['-groups-tree-'].Widget.item(key_to_id(key, window['-groups-tree-']), open=True)
                                window['-СollapseExpandGroup-'].metadata = 'collapse'
                                window['-СollapseExpandGroup-'].TooltipObject.text = 'Свернуть'
                        # if event == '-groups2-':
                        #     if values['-groups2-']:
                        #         if filter_status_group:
                        #             # noinspection PyUnboundLocalVariable
                                                    ...
                                                    if val_modify_user['UserModifyLogin'] == 'admin':
                                                        ...
                                                        try:
                                                            if ip != '127.0.0.1':
                                                                ...
                                                                path_pwd = "/home/" + USERNAME + "/Omega/.admPWD_b"
                                                                with open(path_pwd, mode='w') as f_ch_pwd:
                                    ...
                                    if not is_org:
                                        ...
                                        for group in groups_from_db:
                                            if group['id'] == selected_group_id:
                                                group_to_change = copy.deepcopy(group)
                                        ...
                                        try:
                                            ...
                                            if res_add_del.status_code == 200:
                                                ...
                                                logging.error(f'Добавление и удаление групп НЕ выполнено '
                                                              f'для {get_user_name_by_id_from_db(selected_user_id)} - {res_add_del.status_code}')
                        ...
                        if event == "Apply2":
                            ...
                            if not values['-groups-tree-']:
                                ...
                                groups_from_db = get_groups_from_db()
                                if filter_status_group:
                                    group_id = filtered_groups_list_of_dict[values['-groups-tree-'][0]]
                                else:
                                    if not is_org:
                                        selected_group_id = str(values['-groups-tree-'][0]).partition('.')[0]
                                        print(selected_group_id)
                                ...
                                for us_id in current_users_ids:
                                    if us_id in tree2.metadata:
                                        ...
                                        print(f"В группе {group_id['name']} нужно удалить пользователя "
                                              f"{get_user_name_by_id_from_db(us_id)}")
                                ...
                                if add_user or del_user:
                                    try:
                                        ...
                                        if res_add_del.status_code == 200:
                                            current_db += 1
                                            logging.info(
                                                f'Изменение пользователей выполнено для {group_id["name"]}')
                                            ...
                                            logging.error(
                                                f'Изменение пользователей НЕ выполнено для {group_id["name"]} - '
                                ...
                                if ev_add_lic == 'Загрузить':
                                    ...
                                    if output.find('USAGE') == -1 and output.rstrip('\n') != 'Validation Failed':
                                        ...
                                        if ip != '127.0.0.1':
                                            ...
                                            path_lic_state = "/home/" + USERNAME + "/Omega/.licenseState"
                                            with open(path_lic_state, mode='w') as f_lic_st:
                                                f_lic_st.write("5")
                                                print("файл записан")
                        ...
                        if event == 'Организации':
                            additional_window = True
                            window_organizations = make_organizations()
                            while True:
                                    ...
                                    if validate('add_user'):
                                        ...
                                        try:
                                            ...
                                            if res_add_user.status_code == 200:
                                                ...
                                                my_popup("Пользователь не добавлен! Ошибка: " + str(res_add_user.status_code) + ". " + res_add_user.text)
                                        except Exception as e:
                            ...
                            if not values['-users-tree-']:
                                ...
                                if selected_user['login'] == 'admin':
                                    ...
                                    window_del_user = make_confirm_window('Вы уверены, '
                                                                          'что хотите удалить пользователя ' + selected_user[
                                                                              'name'] + '?')
                                    while True:
                                        ...
                                        if ev_del_user == 'okExit':
                                            try:
                                                ...
                                                if res_del_user.status_code == 200:
                                                    current_db += 1
                                                    logging.info(f'Пользователь {selected_user["name"]} удалён')
                                                    update_all()
                                                    # if filter_status:
                                                    #     search_str = values['-filterUser-']
                                        ...
                                        if validate('clone_user'):
                                            ...
                                            try:
                                                ...
                                                if res_clone_user.status_code == 200:
                                                    current_db += 1
                                                    logging.info(f"Новый пользователь {selected_user['name']} клонирован")
                        ...
                        if event == 'Tabs':
                            if values['Tabs'] == 'Tab3':
                                ...
                                window['countLogs'].update(count_string)
                                window['rowLimit'].update('Лимит ' + str(LOG_DEPTH) + ' строк')
                            elif values['Tabs'] == 'Tab1':
                                treedata_empty = sg.TreeData()
                                if not values['-users-tree-']:
                                    tree.update(treedata_empty)
                            elif values['Tabs'] == 'Tab2':
                                treedata_empty = sg.TreeData()
                                if not values['-groups-tree-']:
                                    tree2.update(treedata_empty)
                        if event == 'info':
                            if filter_journal_info:
