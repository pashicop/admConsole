    ...
    tab1_journal_test = [
        [sg.Column([[
            ...
            sg.Input(size=(15, 1),
                     enable_events=True,
                     disabled_readonly_background_color=disabled_input,
                     key='-filterJournalTest-'),
            sg.Button('', disabled_button_color='white',
                      image_data=ICON_CLEAR_FILTER_BASE_64_BLUE,
                      button_color='white',
                      tooltip='Очистить фильтр',
                      key='-ClearFilterJournalTest-',
                      disabled=True,
                      pad=((0, 5), 0))]],
                      vertical_alignment='bottom'),
            sg.Push(),
            sg.Button('Получить', key='-GetLogTest-'),
            sg.Button('', disabled_button_color='white',
                      image_data=ICON_SAVE_BASE_64_BLUE,
                      button_color='white',
                      # initial_folder='.',
        ...
        [sg.Multiline(key='journalTest', write_only=True, disabled=True, expand_x=True, expand_y=True,
                      autoscroll=True,
                      auto_refresh=True,
                      background_color='white'),
         sg.Frame('Типы',
                  [
                      [sg.Checkbox('Инфо', enable_events=True, key='infoTest', default=True)],
                      [sg.Checkbox('Предупреждение', enable_events=True, key='warningTest', default=True)],
                      [sg.Checkbox('Ошибка', enable_events=True, key='errorTest', default=True)],
                      [sg.Checkbox('Критическая', enable_events=True, key='criticalTest', default=True)],
                  ],
                  size=(150, 140),
                  vertical_alignment='top')
         ],
        [sg.T('Количество записей:'), sg.Multiline(key='countLogsTest', no_scrollbar=True,
                                                   write_only=True, disabled=True, auto_refresh=True,
                                                   background_color='lightgray',
                                                   # auto_size_text=True,
                                                   size=15,
        ...
        [sg.Frame('Логи',
                  [
                      [sg.TabGroup([[sg.Tab('Локальный', tab1_journal, key="Tab1_journal")],
                                    [sg.Tab('Локальный тест', tab1_journal_test, key="Tab1_journal_test")],
...
def check_server(url_ping):
    ...
    if res_ping == '':
        ...
        logging.critical(f'Сервер НЕ доступен при запуске приложения')
    else:
        if res_ping.status_code == 200:
            status['run'] = True
...
def get_settings(url):
    ...
    if res == '':
        print('Сервер не отвечает')
        logging.error(f'Сервер НЕ доступен при запуске приложения')
...
def get_logs_test():
    try:
        with open('admin.log', mode='r', encoding='cp1251') as log_f:
            ...
            got_log = True
            # filtered_journal = []
            # if len(journal_list) - 1 == log_last_num
            ...
            for i, raw in enumerate(journal_list):
                ...
                if i >= len(journal_list) - LOG_DEPTH:
                    if 'CRITICAL' in raw or 'ERROR' in raw:
                        if cur_color == '#ff0001':
                            log_string += journal_list[i] + "\n"
                        else:
                            window['journalTest'].update(log_string, text_color_for_value=cur_color,
                                                         background_color_for_value=cur_back, append=True)
                            refresh_count += 1
                            cur_color = '#ff0001'
                            cur_back = 'black'
                            log_string = journal_list[i] + "\n"
                        ...
                        if cur_color == 'black':
                            ...
                            cur_back = 'white'
                            log_string = journal_list[i] + "\n"
                    if (i + 1) == len(journal_list) and log_string:
                        window['journalTest'].update(log_string, text_color_for_value=cur_color,
                                                     background_color_for_value=cur_back, append=True)
                        refresh_count += 1
                    # if 'INFO' in raw:
                    #     # window['journalTest'].print(journal_list[i], c="white on blue")
                    #     if cur_color == 'blue':
                    #         log_string += journal_list[i] + "\n"
                    #     else:
                    #         window['journalTest'].update(log_string, text_color_for_value=cur_color,
            ...
            # window['journalTest'].update("\n".join(journal_list[-LOG_DEPTH:]))
            # return ["\n".join(journal_list[-LOG_DEPTH:]), len(journal_list), len(journal_list)]
    except Exception as e:
...
def update_logs_test():
    try:
        with open('admin.log', mode='r', encoding='cp1251') as log_f:
            ...
            refresh_count = 0
            global log_last_num
            for i in range(log_last_num + 1, len(journal_list)):
                if i >= len(journal_list) - LOG_DEPTH:
                    journal_list[i] = str(i + 1).ljust(6) + ' ' + journal_list[i]
                    if 'CRITICAL' in journal_list[i] or 'ERROR' in journal_list[i]:
                        if cur_color == '#ff0001':
                            log_string += journal_list[i] + "\n"
                        else:
                            window['journalTest'].update(log_string, text_color_for_value=cur_color,
            ...
            # global log_last_num
            log_last_num = i
            print(f'refresh_count update = {refresh_count}')
    except Exception as e:
        print(f'Не удалось получить локальные логи - {e}')
        logging.error(f'Не удалось получить локальные логи - {e}')
...
def get_logs():
    try:
        with open('admin.log', mode='r', encoding='cp1251') as log_f:
            ...
            # print(f'Время обработки логов - {str(elapsed)}')
                ...
                while True:
                    ...
                    while True:
                        ...
                        if event == '-GetLogTest-':
                            ...
                            # window['journalTest'].update(auto_refresh=True)
                            print(f'Время обработки раскрашенных логов - {str(elapsed_time)}')
                        if event == 'Tabs_journal':
                            ...
                            if values['Tabs_journal'] == 'Tab1_journal':
                                ...
                                window['journal'].update(output_text[0])
                                elapsed_time = timeit.default_timer() - start_time
                                print(f'Время обработки логов - {str(elapsed_time)}')
                                count_string = str(output_text[1]) + ' из ' + str(output_text[2])
                                window['countLogs'].update(count_string)
