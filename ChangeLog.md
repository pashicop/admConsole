...
2.0.0
* Сохранение ip и логина при нажатии чекбокса Запомнить данные в конфигурационном файле `config/app.json`
* Сохранение логина и порта для доступа по ssh к серверу (для остановки, запуска сервиса, скачивания лога сервера, установки лицензии и тд) в конфигурационном файле `config/app.json`
* Пароль от ssh хранится в runtime. После перезагрузки приложения нужно ввести пароль ssh заново
* Отображение изменение логина, пароля и порта ssh в настройках
...
* Отображение в статус баре Роли сервера (Основной или резервный)
...
* Скрипт для установки ws версии (установка redis-server)
* Fix Если в онфиге не указан логин и порт, то пробуем с дефотным портом и логином и паролем. Если в конфиге альтернативный порт, то используем его
* Добавление отображения лимита отображаемых строк (меняется в настройках) 
* Добавмл в настройки включение и отключение функций Скрытое прослушивание, Авторизация устройств, Динамические группы, Геопозиционирование, Долгое скрытое прослушивание, Контроль пересылки, Удалённое программирование терминалов, Макс время прослушивания 
...
* Отправка файла .apk на сервер
* fix отключении сервера убрал запрос лицензии, вызывающий ошибку и попап окно. Оставил только при появлении сервера
* Обновления приложений поддерживают типы
* Готовое окно с обновлениями
* Возможность изменения обновления
* Возможность удаления обновления


2.0.2
* Добавил столбец Блок во вкладке пользователи для таблицы Групп
* Фикс изменения ролей

2.0.3
* Добавил пункт `Устройства` в меню `Сервер`
* Добавил Организации, пока задизейблил
* Добавил возможность смены отображения в виде дереве от пользователей и устройств, на которых авторизовывались к устройствам и пользователям, которые на нём авторизовывались
* Добавил в устройствах возможность разворачивать и сворачивать деревья
* fix клонирование пользователя
* организации для пользователей 
* организации для групп 
* дерево для пользователей
- дерево для групп
- изменения пользователей и групп при работе с деревом