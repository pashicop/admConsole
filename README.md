...
**Необходимые версии:**
* Сервер >= 2.5.0.4
* Диспетчер >= 2.5.0.1.0.23
* Моб. клиент >= 2.5.0.3.8.2

**Обновлено**
* Сортировка устройств по времени последнего обновления состояния заряда батареи
* Сортировка типов приложения обновления по типу 'Основная'-'Тональник'-'Без экрана'-'К500'-'ZTE'
* Добавлена информация к текущему пользователю устройства, чтобы увидеть под каким пользователем сейчас авторизовались на устройстве, если их было зарегистрировано несколько

**Исправлено**
* Исправление ошибки при удалении пользователя
...
**Обновлено**
* Сортировка обновлений по версии
* Группировка обновлений по типу приложений 
**Исправлено**

**Новое**
* Добавление аватаров группе 
* Изменение аватаров группе
* Добавление аватаров пользователю 
* Изменение аватаров пользователю
...
**Необходимые версии:**
* Сервер >= 2.0.2.4
* Сервер WS - ждём интеграции обновлений в ветку с WS
* Диспетчер >= 2.1.3
* Моб. клиент >= 2.1.5
...
**Обновлено**
* Доработка скрипта установки для работы под любым пользователем (не omega)
* Обновил скрипт для установки ws версии (установка redis-server)
* Подсвечивание кнопки `Загрузить` при выборе валидной лицензии
* Используем свежую библиотеку requests -> 2.31.0
* Поменял цвет галочки на цвет из темы
* Добавление отображения лимита отображаемых строк логов(меняется в настройках) 
* Файл создания локальной БД теперь в коде
* Папки Logs и Config создаются автоматически, если их нет

**Исправлено**
* Fix Если в конфиге не указан логин и порт, то пробуем с дефотным портом и логином и паролем. Если в конфиге альтернативный порт, то используем его
* Fix отключении сервера убрал запрос лицензии, вызывающий ошибку и попап окно. Оставил только при появлении сервера

**Новое**
* Сохранение ip и логина в конфигурационном файле `config/app.json` при нажатии чекбокса `Запомнить данные`
* Фокус на первое незаполненное поле при логине, если есть файл конфига с заполненными данными
* Сохранение логина и порта для доступа по ssh к серверу в конфигурационном файле `config/app.json` (для остановки, запуска сервиса, скачивания лога сервера, установки лицензии и тд)
  + По умолчанию пытаемся подключиться с логином/паролем omega/omega12345, если не получается - предлагаем ввести новые данные
  + Пароль от ssh хранится в runtime. После перезагрузки приложения нужно ввести пароль ssh заново, если логин/пароль не omega/omega12345!
  + Отображение изменения логина, пароля и порта ssh в меню настроек из конф. файла и при успешном логине
  + Измение логина, пароля и порта ssh в меню настроек
* Адрес сервера работает с dns именами.
* Также можно прописывать альтернативный порт в строке адреса сервера (:_порт_). Если не указан, берём 5000
* Отображение в статус баре Роли сервера (Основной или резервный). Настраивается на сервере в `appsettings.json`
* Переход на python 3.10.12
* Добавил в настройки включение и отключение функций Скрытое прослушивание, Авторизация устройств, Динамические группы, Геопозиционирование, Долгое скрытое прослушивание, Контроль пересылки, Удалённое программирование терминалов, Макс время прослушивания
* Добавил список ролей в окне изменения пользователя
* Функция Привязать новое устройство (роль №13) реализована через кнопку
* Окно обновлений приложения
  + Отображение версии сервера в окне обновлений
  + Отправка файла .apk на сервер с метаданными (Версия и тип парсится из имени файла)
  + Возможность изменения обновления 
  + Возможность удаления обновления
